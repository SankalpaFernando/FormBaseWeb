...
const NewProject:React.FC<NewProjectProps>=({successCallback})=>{
  ...
  const form = useForm({
    ...
    validate: {
      name: (value: string) => (value.length < 5 ? "Project Name should at least contain 5 characters" : null),
      description: (value: string) => (value.length < 50 ? "Project Description should at least contain 50 characters" : null)
    }
  })
  const onSubmit = () => {
    ...
    <Grid>
      ...
      <Grid.Col>
        <InputWrapper
          ...
          required
        >
          <Textarea id="projectDescription" {...form.getInputProps("description")} />
        </InputWrapper>
      </Grid.Col>
      <Grid.Col>
        <div
        ...
        >
          <Button loading={isLoading} onClick={onSubmit}>Add Project</Button>
        </div>
      </Grid.Col>
    </Grid>
  );
